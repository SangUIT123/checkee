...
const updateAppendixContractSuccess = (draft: any, { data }: any) => {
  draft.appendixContractData[draft.appendixContractData.findIndex(appendixAppendixContract => appendixAppendixContract._id === data._id)] = data;
  draft.error = null
}
...
const deleteAppendixContractSuccess = (draft: any, {data}: any) => {
  draft.appendixContractData = draft.appendixContractData.filter(appendixAppendixContract => appendixAppendixContract._id !== data.id)
  draft.error = null
}
  ...
  return produce(state, (draft) => {
    switch(action.type) {
      ...
      case appendixContractActionTypes.CREATE_APPENDIX_CONTRACT_SUCCESS: 
        createAppendixContractSuccess(draft, action.payload)
        requestDone(draft)
        break;
      ...
      case appendixContractActionTypes.DELETE_APPENDIX_CONTRACT_SUCCESS: 
        deleteAppendixContractSuccess(draft, action.payload)
        requestDone(draft)
        break;
