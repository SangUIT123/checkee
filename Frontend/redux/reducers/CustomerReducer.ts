...
const updateCustomerSuccess = (draft: any, { data }: any) => {
  draft.customerData[draft.customerData.findIndex(customer => customer._id === data._id)] = data;
  draft.error = null
}
...
const deleteCustomerSuccess = (draft: any, {data}: any) => {
  draft.customerData = draft.customerData.filter(customer => customer._id !== data.id)
  draft.error = null
}
  ...
  return produce(state, (draft) => {
    switch(action.type) {
      ...
      case customerActionTypes.CREATE_CUSTOMER_SUCCESS: 
        createCustomerSuccess(draft, action.payload)
        requestDone(draft)
        break;
