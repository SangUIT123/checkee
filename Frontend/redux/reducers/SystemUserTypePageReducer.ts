...
const successAddSystemUserTypePage = (draft: any, { data }: any, isLoading : boolean ) => {
  draft.systemUserTypePageList = data;
  draft.isLoading = isLoading;
};

const failureAddSystemUserTypePage = (draft: any, { error }: any, isLoading : boolean ) => {
  draft.error = error;
  draft.isLoading = isLoading;
};
...
const successUpdateSystemUserTypePage = (draft: any, { data }: any, isLoading : boolean ) => {
  draft.systemUserTypePageList = data;
  draft.isLoading = isLoading;
};

const failureUpdateSystemUserTypePage = (draft: any, { error }: any, isLoading : boolean ) => {
  draft.error = error;
  draft.isLoading = isLoading;
};
...
const successDeleteSystemUserTypePage = (draft: any, { data }: any, isLoading : boolean ) => {
  draft.systemUserTypePageList = data;
  draft.isLoading = isLoading;
};

const failureDeleteSystemUserTypePage = (draft: any, { error }: any, isLoading : boolean ) => {
  draft.error = error;
  draft.isLoading = isLoading;
};
  ...
  return produce(state, (draft) => {
    switch (action.type) {
      ...
      case actionTypes.ADD_SYSTEMUSERTYPEPAGE_SUCCESS:
        let dataAdd = [...state.systemUserTypePageList];
      ...
      case actionTypes.UPDATE_SYSTEMUSERTYPEPAGE_SUCCESS:
        ...
        let res = dataUpdate.map(
          (obj) => [action.payload.data].find((o) => o._id === obj._id) || obj
        );
        return {
          ...state,
      ...
      case actionTypes.CHECK_ACCESSRIGHT_SUCCESS:
        ...
        let dataCheckAR = [...state.accessRightData];
        ...
        const i = dataCheckAR.findIndex(_item => _item.actionName === object.actionName);
