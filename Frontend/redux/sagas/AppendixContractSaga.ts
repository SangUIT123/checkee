...
import deleteAppendixContract from '../../constant.config.api/api/deleteAppendixContract'
...
export function* createAppendixContractProcess(action) {
  try { 
    ...
    yield put(createAppendixContractSuccess(response.data.data))
    NProgress.done()
...
export function* deleteAppendixContractProcess(action) {
  try {
    const response = yield call(deleteAppendixContract, action.payload)
    yield put(deleteAppendixContractSuccess(response.data.data))
    NProgress.done()
...
export function* onGetAppendixContractStart() {
  yield takeLatest(actionTypes.GET_APPENDIX_CONTRACT_START, getAppendixContractProcess)
}

export function* onCreateAppendixContractStart() {
  yield takeLatest(actionTypes.CREATE_APPENDIX_CONTRACT_START, createAppendixContractProcess)
}

export function* onUpdateAppendixContractStart() {
  yield takeLatest(actionTypes.UPDATE_APPENDIX_CONTRACT_START, updateAppendixContractProcess)
}

export function* onDeleteAppendixContractStart() {
  yield takeLatest(actionTypes.DELETE_APPENDIX_CONTRACT_START, deleteAppendixContractProcess)
}
