...
import { createCustomerStart, updateCustomerStart } from "Frontend/redux/actions/CustomerActions";
...
export function CertificateRow(props) {
  ...
  const imageUploadEl = React.useRef(null)
    ...
    return new Promise((resolve, reject) => {
        ...
        if(file) {
          ...
          reader.onload = (readerEvt) => {
            ...
            let base64 = 'data:image/' + mimeType + ';base64,' + btoa(binaryString)
            //base64
            resolve(base64)
          }
  ...
  return (
    <React.Fragment>
      <Box boxShadow={1} padding={1} borderRadius={5} margin={2} style={{ position: 'relative'}}>
        <Grid container className={classes.paper} style={{ marginLeft: '20px', boxSizing: 'border-box'}}>
            <GridList className={classes.gridList} cols={4} spacing={10} cellHeight={80}>
              {
                certs.map((img, index) => (
      ...
      <Typography color={totalSize > LIMIT ? 'error' : 'primary'} variant={'caption'} style={{ marginLeft: '20px'}}>Tổng kích thước: { Math.abs(totalSize / 1000).toFixed(2) } MB</Typography>
      {
        (currentError && currentTouched) && <Typography color={'error'} variant={'caption'} style={{ marginLeft: '20px'}}>{currentError}</Typography>
      }
    </React.Fragment>
  )
}
...
export default function Customer_Form(props){
  ...
  const [certificates, setCertificates] = React.useState(customer ? customer.certificates: [])
  const requestState = useSelector(({ Customer }: any) => Customer) || null // get error and working state

  const initialFormValues = {
    id: customer ? customer._id : "",
    name_customer: customer ? customer.name_customer : "huiqdwuq",
    password: !checkEdit ? customer ? customer.password : "Huyuyuyuy1231293" : null,
    taxCode: customer ? customer.taxCode : "1234567890",
    bank: customer ? customer.bank : "VCB", 
    account_number: customer ? customer.account_number : "1234567890", 
    address: customer ? customer.address : "wqdwqdqw", 
    phone: customer ? customer.phone : "1234567890", 
    fax: customer ? customer.fax : "1234567890",
    email: customer ? customer.email : Math.random()*1000 + "@fm.com",
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        >
          {(props) => (
          <form onSubmit={props.handleSubmit} onChange={() => setFormChanged(true)}>
              {/* {
              ...
              <DialogContent>
                <Grid container>
                      ...
                      <Grid item xs={12} className={classes.gridItem}>
                        <InputLabel id="" style={{ marginLeft : "10px"}}>
                          ...
                          <TextField
                            ...
                            error={
                              props.touched.name_customer && Boolean(props.errors.name_customer)
                            }
                            helperText={props.touched.name_customer && props.errors.name_customer}
                          />
                      </Grid>
                      <Grid item xs={12} className={classes.gridItem}>
                          <InputLabel id="" style={{ marginLeft : "10px"}}>Mã số thuế</InputLabel>
                            <TextField
                              size="small"
                              name="taxCode"
                  ...
                  <Grid item xs={12} sm={6} className={ classes.gridColumn }>
                    <Grid item>
                      <Grid item xs={12} className={classes.gridItem}>
                        <InputLabel id="" style={{ marginLeft : "10px"}}>Fax</InputLabel>
                          <TextField
                            size="small"
                            name="fax"
                            ...
                            onChange={e => handleFieldChange(e,props)}
                            error={
                              props.touched.fax && Boolean(props.errors.fax)
                            }
                      ...
                      <Grid item xs={12} className={classes.gridItem}>
                          <Grid container>
                              ...
                              <InputLabel id="" style={{ marginLeft : "10px"}}>Tài khoản ngân hàng</InputLabel>
                              <TextField
                                size="small"
                                name="account_number"
                                ...
                                onChange={e => handleFieldChange(e,props)}
                                error={
                                  props.touched.account_number && Boolean(props.errors.account_number)
                                }
                                helperText={props.touched.account_number && props.errors.account_number}
                              />
                              </Grid>
                              <Grid item xs={12} sm={5}>
                                <InputLabel id="" style={{ marginLeft: '10px'}}>Ngân hàng</InputLabel>
                                <TextField
                                  size="small"
                                  name="bank"
                      ...
                      {/*-----------------------------------Tạo mới đối tác----------------------------------*/}
                      <Grid item xs={12} className= {classes.gridItem}>
                        <InputLabel id="" style={{ marginLeft: "10px"}}>Chứng chỉ (giới hạn 1MB)</InputLabel>
                        <Field
                          ...
                          validate={(value) => {
                            if(value.length === 0) {
                              ...
                              let totalSize = value.reduce((acc, i) => acc += calcFileSize(i), 0)
                              if(totalSize > 1000) {
