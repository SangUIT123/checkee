...
export default function ManagementMenuFormUpdate(props) {
  ...
  const listLv = useSelector((state: any) => state.SystemPage.systemPageList);
  const listLv0 =
    listLv !== null ? listLv.filter((todo) => todo.level === 0) : [];
  const listLv1 =
    listLv !== null ? listLv.filter((todo) => todo.level === 1) : [];
  const listLv2 =
    listLv !== null ? listLv.filter((todo) => todo.level === 2) : [];
  //
  ...
  //
  const { openModalUpdate, closeModalUpdate } = props;
  const classes = useStyles();
  // console.log("vald", props.dataUpdate);
