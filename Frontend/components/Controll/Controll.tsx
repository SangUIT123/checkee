...
export default function Controll() {
  const [systemPageList, setSystemPageList] = useState([]);
  const [userTypeList, setUserTypeList] = useState([]);
  const [systemUserTypePageList, setSystemUserTypePageList] = useState([]);
  const [systemPage, setSystemPage] = useState(null);
  const [systemPageModule, setSystemPageModule] = useState([]);
  const [systemPageFunction, setSystemPageFunction] = useState([]);
  const [systemPageFeature, setSystemPageFeature] = useState([]);
  ...
  const emptyRows =
    rowsPerPage -
    Math.min(rowsPerPage, rows ? rows.length - page * rowsPerPage : 0);
