...
import { getAppendixContractStart, deleteAppendixContractStart, IAppendixContract } from '../../redux/actions/AppendixContractActions'
...
function AppendixContract() {
  ...
  const appendixContractData: IAppendixContract[] = useSelector((state: any) => state.AppendixContract.appendixContractData) || []
  const [AppendixContract, setContract] = useState(null);
  ...
  const [
    ...
    setCheckCreateAppendixContract,
  ] = React.useState(false);
  const [
    checkUpdateAppendixContract,
    setCheckUpdateAppendixContract,
  ] = React.useState(false);
  const [
    checkDeleteAppendixContract,
    setCheckDeleteAppendixContract,
  ] = React.useState(false);
  // const dispatch = useDispatch();
  ...
  useEffect(() => {
    function loadAppendixContract() {
      dispatch(getAppendixContractStart())
    }

    loadAppendixContract()
  }, [])

  useEffect(() => {
    console.log(appendixContractData)
  }, [appendixContractData])

  const emptyRows =
    rowsPerPage - Math.min(rowsPerPage, appendixContractData.length - page * rowsPerPage);
                    ...
                    ? (rowsPerPage > 0
                        ...
                        <TableRow key={index}>
                          ...
                          <TableCell
                            ...
                            className={classes.tableRightBorder}
                          >
                            {new Date(row.date).toLocaleString().split(',')[0]}
                          </TableCell>
                          <TableCell
                            component="th"
                            scope="row"
                            className={classes.tableRightBorder}
                          >
                            {new Date(row.startDay).toLocaleString().split(',')[0]}
                          </TableCell>
                          <TableCell
                            component="th"
                            scope="row"
                            className={classes.tableRightBorder}
                          >
                            {new Date(row.endDay).toLocaleString().split(',')[0]}
                          </TableCell>
                          <TableCell
                            component="th"
                            scope="row"
                            className={classes.tableRightBorder}
                          >
                            {new Date(row.durationPay).toLocaleString().split(',')[0]}
                          </TableCell>
                          <TableCell
                            component="th"
                            scope="row"
                ...
                <TableFooter>
                  <TableRow>
                    <TablePagination
                      ...
                      page={page}
                      SelectProps={{
                        inputProps: { "aria-label": "appendixContractData per page" },
                        native: true,
                      }}
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            <span>
              Bạn có chắc chắn muốn xóa "<b>{AppendixContract?.name}"</b>?
            </span>
          </DialogContentText>
        </DialogContent>
        <DialogActions>
...
/*
  ...
  const appendixContractData: Array<ContractProps> = [{
    orderNo: 1,
